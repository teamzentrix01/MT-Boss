  ...
  return (
    <article className="hr-card">
      ...
      <div className="hr-card-body">
        ...
        {/* Category · Unit */}
        <p className="hr-card-cat">{product.category.name}{product.unit ? ` · ${product.unit}` : ""}</p>
        ...
        <div className="hr-card-price">
          {hasPrice ? (
            <>
              ...
              <span className="hr-price-unit"> / {product.unit || "unit"}</span>
            </>
          ) : (
        ...
        <div className="store-overlay" onClick={() => setDetailsProduct(null)}>
          ...
          >
            ...
            <div className="store-detail-info">
              ...
              <p>{detailsProduct.category.name} · {detailsProduct.unit}</p>
              <div className="store-detail-price">
                {Number(detailsProduct.price) > 0
                  ? <>₹{Number(detailsProduct.price).toLocaleString("en-IN")} / {detailsProduct.unit}
                ...
                <><h3>Bulk prices</h3>
                  <div className="store-bulk-tiers">
                    {detailsProduct.bulk_pricing.map((t) => (
                      <span key={t.min_quantity}>{t.min_quantity}+ {detailsProduct.unit}: ₹{Number(t.price).toLocaleString("en-IN")} each</span>
                    ))}
                  </div></>
              )}
              <div className="store-detail-actions">
                <button type="button" onClick={() => { onQuote(detailsProduct); setDetailsProduct(null); }}>Get Quote</button>
                <button type="button" onClick={() => { onBuy(detailsProduct); setDetailsProduct(null); }}>Buy Now</button>
                <button type="button" onClick={() => onAdd(detailsProduct)}>Add to cart</button>
              </div>
            </div>
          </section>
        </div>
              ...
              <>
                  ...
                  {cart.map((item) => (
                    <div className="store-cart-item" key={item.product.id}>
                      ...
                      <div>
                        ...
                        <span>{item.product.category.name} · {item.product.unit}</span>
                ...
                <div className="store-cart-bottom">
                  <div className="store-cart-estimate">
                    ...
                    <strong>{pricedTotal ? `₹${pricedTotal.toLocaleString("en-IN")}${hasUnpriced ? " + quote items" : ""}` : "On request"}</strong>
                  </div>
                  <p>Final pricing and delivery are confirmed by the supplier.</p>
