    ...
    <main className={`min-h-screen py-24 px-6 transition-colors duration-500 ${isDark ? "bg-black" : "bg-zinc-50"}`}>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          {/* Left Side: Info */}
          <div>
            <p className="text-[var(--brand-blue)] text-xs font-black uppercase tracking-[0.4em] mb-4">Engineering Estimate</p>
            <h1 className={`text-5xl md:text-7xl font-black uppercase tracking-tighter leading-none mb-8 ${isDark ? 'text-white' : 'text-zinc-900'}`}>
              Request a <br /> <span className="text-[var(--brand-blue)]">Quote</span>
            </h1>
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={i} className="flex items-center gap-4">
                  <div className="w-1.5 h-1.5 bg-[var(--brand-blue)] rotate-45" />
                  <div>
          ...
          {/* Right Side: Form */}
          <div className={`p-8 md:p-12 border-t-8 border-[var(--brand-blue)] shadow-2xl ${isDark ? 'bg-zinc-900' : 'bg-white'}`}>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="text" name="name" required onChange={handleChange} className={`w-full p-4 text-sm font-bold border-b-2 outline-none transition-all ${isDark ? 'bg-zinc-800 border-zinc-700 text-white focus:border-[var(--brand-blue)]' : 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-black'}`} />
                </div>
                <div>
                  ...
                  <input type="email" name="email" required onChange={handleChange} className={`w-full p-4 text-sm font-bold border-b-2 outline-none transition-all ${isDark ? 'bg-zinc-800 border-zinc-700 text-white focus:border-[var(--brand-blue)]' : 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-black'}`} />
                </div>
              </div>
              ...
              <div>
                ...
                <input type="text" name="budget" placeholder="e.g. 50 Lakhs" onChange={handleChange} className={`w-full p-4 text-sm font-bold border-b-2 outline-none transition-all ${isDark ? 'bg-zinc-800 border-zinc-700 text-white focus:border-[var(--brand-blue)]' : 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-black'}`} />
              </div>
              ...
              <div>
                ...
                <textarea name="message" rows="4" required onChange={handleChange} placeholder="Describe your vision..." className={`w-full p-4 text-sm font-bold border-b-2 outline-none transition-all resize-none ${isDark ? 'bg-zinc-800 border-zinc-700 text-white focus:border-[var(--brand-blue)]' : 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-black'}`}></textarea>
              </div>

              <button type="submit" className="w-full py-5 bg-[var(--brand-blue)] text-black font-black uppercase text-xs tracking-[0.3em] hover:bg-white hover:shadow-lg transition-all active:scale-95">
                Send Request
              </button>
            </form>
          </div>
