...
export async function GET(req, { params }) {
  try {
    ...
    const [agentResult, leadsResult, scheduleResult, activityResult, projects] = await Promise.all([
      pool.query(
    ...
    return NextResponse.json({
      ...
      unread_activity_count: activityResult.rows.reduce((count, item) => count + (item.is_read ? 0 : 1), 0),
      projects,
    });
  } catch (error) {
