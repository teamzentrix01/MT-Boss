...
export default function SellPage() {
  ...
  const steps = ["Property Details", "Your Info", "Photos", "Done"];
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        {step === 1 && (
          <div className={`border rounded-sm p-6 space-y-5 ${card}`}>
            ...
            <div><label className={lbl}>Property Title *</label><input className={inp} placeholder="e.g. 3BHK Villa in Noida" value={form.title} onChange={e=>set("title",e.target.value)} /></div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={lbl}>Property Type *</label>
                <input className={inp} list="sell-property-types" value={form.type}
                  onChange={e=>set("type",e.target.value)} placeholder="Select or type property type" />
                <datalist id="sell-property-types">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><label className={lbl}>Price (₹) Display *</label><input className={inp} placeholder="e.g. 85,00,000" value={form.price} onChange={e=>set("price",e.target.value)} /></div>
        ...
        {submitted && (
          <div className={`border rounded-sm p-10 text-center space-y-4 ${card}`}>
            ...
            <p className={`text-sm leading-relaxed ${muted}`}>
              Your property has been submitted for admin verification. It will go live on the Buy Properties page after approval.
            </p>
            <div className={`border rounded p-4 text-left space-y-1 ${dark ? "border-zinc-800":"border-gray-100"}`}>
              ...
              <p className={`text-xs ${muted}`}>✓ Once approved, listing goes live with all images</p>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button onClick={() => { setSubmitted(false); setSubmittedListing(null); setStep(1); setForm(empty); setImages([]); setImageFiles([]); }}
